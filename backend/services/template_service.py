...
【输出格式要求】
1. 标题层级：使用 Markdown 标题表示结构——
   - 一级标题（#）：报告总标题，如 “# 事故分析报告”
   - 二级标题（##）：各章节标题，章节编号保留在标题文字中，如 “## 一、基本情况”
2. 正文：每个自然段独立成行，段落之间空一行。
3. 正文内不得使用 #、*、-、> 等 Markdown 符号（标题除外）；如需次级列举，用（一）1.（1）作普通文本行内编号，不用标题样式。
4. 章节弹性：若材料不足以支撑某一章节，可精简或合并，但六个核心维度应尽量覆盖。各章节标题可根据材料特点，从上方”通用结构参考”中选择最贴切的变体，不要求完全一致。
